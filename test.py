...
# Create a header with radio buttons for the menu
menu = st.radio("Menu", ("Show Data", "List of Crimes and Victims"))
...
elif menu == "List of Crimes and Victims":
    # Display a table of crimes and their victims for each crime type
    st.header("List of Crimes and Victims")
    
    # Get unique crime types
    unique_crime_types = crime_type_counts["crime_type"]
    
    for crime_type in unique_crime_types:
        st.subheader(f"Crime Type: {crime_type}")
        
        # Create a DataFrame for the current crime type
        crime_type_df = df[df["crime_type"] == crime_type]
        
        # Filter out rows with missing or vacant information
        crime_type_df = crime_type_df.dropna(subset=['Name','phone','email','location'])
        
        # Reset the index for correct numbering
        crime_type_df = crime_type_df.reset_index(drop=True)
        crime_type_df.index = crime_type_df.index + 1 
        
        # Create a table for the current crime type
        crime_type_table = crime_type_df[['Name','phone','email','location']]
        
        # Display the table
        st.dataframe(crime_type_table, height=400)
